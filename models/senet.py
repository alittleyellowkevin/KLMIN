...
class SENet(nn.Module):
    ...
    def __init__(self, block, layers, groups, reduction, dropout_p=0.2,
                 ...
                 downsample_padding=1, num_classes=1000, loss={'xent'}):
        """
        Parameters
        ----------
...
def senet154(num_classes, loss={'xent'}, pretrained=True, **kwargs):
    model = SENet(SEBottleneck, [3, 8, 36, 3], groups=64, reduction=16,
                  dropout_p=0.2, num_classes=num_classes, loss=loss)
    if pretrained:
        init_pretrained_weights(model, model_urls['senet154'])
    return model
...
def se_resnet50(num_classes, loss={'xent'}, pretrained=True, **kwargs):
    model = SENet(SEResNetBottleneck, [3, 4, 6, 3], groups=1, reduction=16,
                  ...
                  downsample_kernel_size=1, downsample_padding=0,
                  num_classes=num_classes, loss=loss)
    if pretrained:
        init_pretrained_weights(model, model_urls['se_resnet50'])
    return model
...
def se_resnet101(num_classes, loss={'xent'}, pretrained=True, **kwargs):
    model = SENet(SEResNetBottleneck, [3, 4, 23, 3], groups=1, reduction=16,
                  ...
                  num_classes=num_classes, loss=loss)
    if pretrained:
        init_pretrained_weights(model, model_urls['se_resnet101'])
    return model
...
def se_resnet152(num_classes, loss={'xent'}, pretrained=True, **kwargs):
    model = SENet(SEResNetBottleneck, [3, 8, 36, 3], groups=1, reduction=16,
                  ...
                  num_classes=num_classes, loss=loss)
    if pretrained:
        init_pretrained_weights(model, model_urls['se_resnet152'])
    return model
...
def se_resnext50_32x4d(num_classes, loss={'xent'}, pretrained=True, **kwargs):
    model = SENet(SEResNeXtBottleneck, [3, 4, 6, 3], groups=32, reduction=16,
                  ...
                  num_classes=num_classes, loss=loss)
    if pretrained:
        init_pretrained_weights(model, model_urls['se_resnext50_32x4d'])
    return model
...
def se_resnext101_32x4d(num_classes, loss={'xent'}, pretrained=True, **kwargs):
    ...
    if pretrained:
        init_pretrained_weights(model, model_urls['se_resnext101_32x4d'])
    return model

if __name__ == "__main__":
    model = se_resnext101_32x4d(num_classes=576)
    print(model)